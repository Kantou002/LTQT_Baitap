...
namespace QuanLyBanHang.Forms
{
    public partial class FrmSanPham : Form
    {
        ...
        {
            // Ensure images folder exists at runtime
            try
            {
            ...
            {
                // If creation fails, continue (file operations will handle errors)
            }
            ...
            // Update binding format to provide fallback when HinhAnh is null/missing
            picHinhAnh.DataBindings.Clear();
            Binding hinhAnh = new Binding("ImageLocation", bindingSource, "HinhAnh");
            hinhAnh.Format += (s, ev) =>
            {
                string fileName = (ev.Value ?? "").ToString();
                if (string.IsNullOrWhiteSpace(fileName))
                    fileName = "no-image.jpg";

                string fullPath = Path.Combine(imagesFolder, fileName);
                string fallback = Path.Combine(imagesFolder, "no-image.jpg");

                if (!File.Exists(fullPath) && File.Exists(fallback))
                    fullPath = fallback;

                ev.Value = File.Exists(fullPath) ? fullPath : "";
            };
        ...
        private void dgvSanPham_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (dgvSanPham.Columns[e.ColumnIndex].Name == "HinhAnh")
            {
                ...
                // fallback to no-image.jpg if needed
                string fallback = Path.Combine(imagesFolder, "no-image.jpg");
                if (!File.Exists(filePath) && File.Exists(fallback))
                    filePath = fallback;

                if (!File.Exists(filePath))
                {
                    // avoid throwing — produce an empty 24x24 image
                ...
                try
                {
                    // Use FromStream to avoid locking the file and ensure disposal
                    using FileStream fs = File.OpenRead(filePath);
                    using Image loaded = Image.FromStream(fs);
                    e.Value = new Bitmap(loaded, 24, 24);
                }
                catch
                {
                    e.Value = new Bitmap(24, 24);
        ...
        private void btnDoiAnh_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    // Ensure target folder exists before attempting copy
                    Directory.CreateDirectory(imagesFolder);

                    // If file already exists, warn and abort
        ...
        {
            string helpFile = Path.Combine(Application.StartupPath, @"Help\HuongDanSuDung.html#sanpham");
            if (!File.Exists(helpFile))
                helpFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\..\Help\HuongDanSuDung.html#sanpham");
